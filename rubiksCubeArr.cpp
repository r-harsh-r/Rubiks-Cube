...
class RubiksCube1dArray : public RubiksCube{
    ...
    RubiksCube1dArray() {
        for (int i = 0; i < 54; i++) {
            cube[i] = RubiksCube::getColorLetter(COLOR(i / 9));
        }
    }

    COLOR getColor(FACE face, unsigned row, unsigned col) const override {
        char col_char = cube[getIndex((int)face, (int)row,(int) col)];
    ...
    bool isSolved() const override{
        for(int i = 0;i<54;i++){
            char color = RubiksCube::getColorLetter(COLOR(i / 9));
            if(cube[i] != color){
                return false;
            }
        }
    ...
    bool operator==(RubiksCube1dArray &rc){
        for(int i = 0;i<54;i++){
            if(cube[i] != rc.cube[i]){
                return false;
            }
    ...
    RubiksCube1dArray & operator=(const RubiksCube1dArray &rc) {
        for(int i = 0;i<54;i++){
            cube[i] = rc.cube[i];
        }
        return *this;
